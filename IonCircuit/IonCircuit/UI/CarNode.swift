...
final class CarNode: SKNode {
    ...
    private func updateExhaust(speed: CGFloat, fwdMag: CGFloat, dt: CGFloat) {
        // No smoke when effectively not moving (and not pressing)
        let moving = speed > 2.0 || throttle > 0.02
        if !moving {
            exhaustL.particleBirthRate = 0
            exhaustR.particleBirthRate = 0
            return
        }

        // Mix: mostly throttle, some speed (feels responsive but stable)
        let speedNorm = CGFloat.clamp(speed / max(maxSpeed, 1), 0, 1)
        let throttleBoost = max(0, throttle)
        let mix = 0.35 * speedNorm + 0.65 * throttleBoost   // 0..1

        // Birth rate & speed
        let baseBR: CGFloat = 80
        let addBR: CGFloat  = 240
        let br = baseBR + addBR * mix

        let basePS: CGFloat = 120
        let addPS: CGFloat  = 160
        let ps = basePS + addPS * mix

        // Scale (slightly larger near max, as before)
        let scale = 0.16 + 0.22 * mix + 0.10 * (mix * mix)

        // NEW: make the trail a bit longer at high speed
        // Lifetime grows gently toward max (≈ +0.10s at mix=1)
        let baseLT: CGFloat = 0.30
        let addLT:  CGFloat = 0.10
        let lt = baseLT + addLT * (mix * mix)              // only stretches near the top
        let ltRange = lt * 0.15                             // subtle variation looks nicer

        // Also fade a touch slower at max so the trail reads longer, but not smoky
        let alphaSpeed = -1.00 + 0.25 * mix                 // -1.00 .. -0.75 (less negative = slower fade)
