...
//  IonCircuit
//
//  Created by Barak Ben Hur on 14/10/2025.
//
...
final class GameOverOverlayNode: SKNode {
    ...
    override init() {
        ...
        panel.path = CGPath(roundedRect: CGRect(x: -w/2, y: -h/2, width: w, height: h), cornerWidth: 16, cornerHeight: 16, transform: nil)
        ...
        button.path = CGPath(roundedRect: CGRect(x: -bw/2, y: -bh/2, width: bw, height: bh), cornerWidth: 12, cornerHeight: 12, transform: nil)
    ...
    func handleTouch(scenePoint p: CGPoint) -> Bool {
        ...
        // Convert the SCENE point into this overlay's local space
        let local = convert(p, from: scn)

        // Slightly larger hit area feels nicer
